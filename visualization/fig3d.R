...
library(tidyverse)
mydata_separated <- mydata %>% separate(quary, into = c("gene", "transcript"), sep = ".t")
...
ggsave(p1,filename = "box_core_gene_copy_number_kaks_v2.pdf",width = 7,height = 6,dpi=300)
library(dplyr)

# 计算每个组的 N 值和标签位置
summary_df <- merged_df %>%
  group_by(sum, Type) %>%  # 根据箱线图的分组变量进行分组
  summarise(
    n = n(),
  ) %>%
  ungroup()
