...
pie<- ggplot(data1, aes(x="", y=Percentage, fill=Group))+#set basic figure
  ...
  geom_text(aes(x=1.2,label = paste(format(round(Percentage*100,1),nsmall=1),"%",sep="")),size=8,color = c("black"),position = position_stack(vjust = 0.5))+ #add text number to pie plot
...
mydata1=colSums(mydata)
write.table(mydata1,"gene_family_number_info_13genome.txt",row.names = T,col.names = T,sep = "\t",quote = F) ##导出处理后的文件
