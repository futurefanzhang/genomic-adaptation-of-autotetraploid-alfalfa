...
##统计124种表达条件中，每一种前10%高表达的基因数量
all_124_exp <- merged_top_info %>%
  group_by(group,group2) %>%
  dplyr::summarise(
    count = n(),  # 统计每个类别的出现次数
  )
write.table(all_124_exp,"all_124_exp_gene_number.txt",row.names = F,col.names = T,sep = "\t",quote = F) ##10% gene number
...
filter_gene2=filter_gene %>% inner_join(copy_info, by=c('gene'='gene'))
write.table(filter_gene2,"203gene_90%condition_high_exp_gene.txt",row.names = F,col.names = T,sep = "\t",quote = F)
...
write.table(result,"leaf_root_exp_four_hap_fpkm.txt", row.names = F,quote=F,sep = "\t")
