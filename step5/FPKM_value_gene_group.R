...
mean_fpkm_fig$Type2 <- str_replace_all(mean_fpkm_fig$Type2, c("one_copy" = "One_copy", "two_copy" = "Two_copy","three_copy" = "Three_copy","four_copy" = "Four_copy"))

mean_fpkm_fig$Type2=factor(mean_fpkm_fig$Type2,levels = c("Onefold_essential", "One_copy", "Two_copy", "Three_copy","Four_copy","Fourfold_core"),ordered = TRUE)
mean_fpkm_fig$mean_fpkm=log2(mean_fpkm_fig$mean_fpkm+1)
p<- ggplot(mean_fpkm_fig, aes(x=Type2, y=mean_fpkm, color=Type2)) + 
  geom_boxplot() +
  scale_color_manual(
...
p=ggplot(filtered_exp_four_pair, aes(x = rank, y = mean_fpkm, color = rank)) +
  geom_boxplot() +
  #geom_smooth(method = "lm", se = TRUE, fullrange = TRUE) +
  labs(title = "",
       x = "Rank",
       y = "FPKM") +theme_classic()+
  theme(
...
ggsave(p,filename = "all_tetra-copy_core_gene_fpkm_order1234.pdf",width = 7,height = 6,dpi=300)
...
p <- ggplot(
  ...
  geom_text(x = 1, y = 4, label = "***", size = 5) +  # 在横线上添加星号
  geom_segment(aes(x = 0.9, xend = 3, y = 3.8)) +  # 添加横线
  geom_text(x = 1, y = 3.8, label = "***", size = 5) +  # 在横线上添加星号
  geom_segment(aes(x = 0.9, xend = 2, y = 3.6)) +  # 添加横线
  geom_text(x = 1, y = 3.6, label = "***", size = 5) +  # 在横线上添加星号
  labs(title = "",
...
ggsave(p,filename = "all_tetra-copy_core_gene_mean_fpkm_order1234.pdf",width = 5,height = 6,dpi=300)
